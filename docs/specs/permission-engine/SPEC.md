...
type Decision struct {
    ...
    Approval *ApprovalRequest // AskOnce/AskAlways 时生成，持久化交 session-store
}

type ApprovalRequest struct {
    SessionID, AgentID, TeamID string
    Source                     string
    ToolName                   string
    ToolSource                 toolruntime.ToolSource
    Input                      json.RawMessage // 脱敏后输入
    Effect                     Effect
    Risk                       RiskLevel
    Reasons                    []RuleHit
...
## 19. Open Questions
- Bash 分析路径已决策：MVP 采用自研保守 lexer，后续如 RISK-006 误判率超阈值再引入完整 shell parser（见 OPEN_QUESTIONS Q16）。
- 敏感目录默认清单的可移植性（跨平台路径）。
