...
## 语言与版本
- 主语言：Go（`go.mod` 使用 Go 1.22 作为兼容基线；当前 Spike 验证环境为 Go 1.26.1）。
- 文档：简体中文；代码标识符、ID、状态枚举、技术术语用英文。

## 目录约定
- 设计文档在 `docs/`（结构见 `docs/README.md`）。
- 实现代码：`cmd/`（入口）、`internal/<module-id>/`（按模块）、`pkg/`（可复用公开包，谨慎使用）。
...
## 命令
- 构建：待建立。
- 测试：`go test ./...`；并发敏感改动需跑 `go test -race ./...`。
- 格式化：`gofmt`。
...
## 当前阶段约束
- 首批实现阶段已启动，按 `docs/planning/TWO_PERSON_WORK_SPLIT.md` 的角色分工执行选定的 Ready/P0 Task。
- 生产代码改动必须绑定明确 Task，并同步对应 `TASKS.md`、`CHECKLIST.md` 与 Evidence。
- 不创建大量空骨架文件；只创建当前 Task 所需的最小代码与测试。
